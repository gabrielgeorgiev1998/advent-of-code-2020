...
'''
input_str = """mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
...
ing_to_alg_map = defaultdict(set)
...
for line in lines:
  ...
  ing_alg_set.append((set(ingredients), set(allergens)))

  all_ings = all_ings.union(ingredients)
  all_algs = all_algs.union(allergens)

for alg in all_algs:
  ing_to_alg_map[alg] = all_ings

for curr_sets in ing_alg_set:
  for alg in curr_sets[1]:
    ing_to_alg_map[alg] = ing_to_alg_map[alg].intersection(curr_sets[0])

final_alg_ing = {}

print(ing_to_alg_map)

while len(final_alg_ing) < len(all_algs):
  ing_to_remove = set()
  for alg, ings in ing_to_alg_map.items():
    if len(ings)==1:
      final_alg_ing[alg] = next(iter(ings))
      ing_to_remove = {final_alg_ing[alg]}
      break
  
  for alg, ings in ing_to_alg_map.items():
    ing_to_alg_map[alg] = ing_to_alg_map[alg] - ing_to_remove
...
alg_count = 0
for alg in final_alg_ing.values():
  alg_count += input_str.count(alg)

print(alg_count)

count = 0
for curr_sets in ing_alg_set:
  count += len(curr_sets[0])

print(count)

print(count - alg_count)
