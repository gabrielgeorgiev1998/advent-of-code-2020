...
def val_all(byr, iyr, eyr, hgt, hcl, ecl, pid):
  print("byr: ", val_byr(byr))
  print("iyr: ", val_iyr(iyr))
  print("eyr: ", val_eyr(eyr))
  print("hgt: ", val_hgt(hgt))
  print("hcl: ", val_hcl(hcl))
  print("ecl: ", val_ecl(ecl))
  print("pid: ", val_pid(pid))
  return(val_byr(byr) and val_iyr(iyr) and val_eyr(eyr) and val_hgt(hgt) and val_hcl(hcl) and val_ecl(ecl) and val_pid(pid))
...
for chunk in chunks:
  ...
  chunk_fields = {}
  for el in chunk:
    chunk_fields[el[:3]] = el[4:]

  if(chunk_fields.keys() == all_fields or len(chunk_fields)==8):
    fake += 1
    if(val_all(chunk_fields["byr"], chunk_fields["iyr"], chunk_fields["eyr"], chunk_fields["hgt"], chunk_fields["hcl"], chunk_fields["ecl"], chunk_fields["pid"])):
      count += 1
