input_str = """clear purple bags contain 5 faded indigo bags, 3 muted purple bags.
...
def get_inner_bags(bag_color):
  bag_line = ""
  for line in input_str.splitlines():
    if(" ".join(line.split()[:2]) == bag_color):
      bag_line = line
      break

  if("no other bags." in bag_line):
    return 1
  else:
    count = 0
    for inner_bag_str in " ".join(bag_line.split()[4:]).split(", "):
      ibs_split = inner_bag_str.split()
      bag_count = int(ibs_split[0])
      bag_color = " ".join(ibs_split[1:3])

      print(bag_count)
      print(bag_color)
      inner_bag_count = bag_count * get_inner_bags(bag_color)
      count += inner_bag_count

    return 1 + count

  # print(" ".join(bag_line.split()[4:]).split(", "))
