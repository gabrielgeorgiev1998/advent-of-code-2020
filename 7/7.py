...
while len(bagstack) > 0:
  curr_bag = bagstack.pop()
  for line in input_str.splitlines():
    line_bag = " ".join(line.split()[:2])
    rest_of_str = " ".join(line.split()[2:])
    if((curr_bag in rest_of_str) and (line_bag not in covered_bags)):
      bagstack.append(line_bag)
      covered_bags.append(line_bag)
      count += 1
